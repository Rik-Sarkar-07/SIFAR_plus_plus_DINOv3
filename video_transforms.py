...
class GroupRandAugment(object):
    def __init__(self, num_ops, magnitude):
        self.worker = torchvision.transforms.RandAugment(num_ops, magnitude)
...
class GroupRandomErasing(object):
    def __init__(self, p=0.25, scale=(0.02, 0.33), ratio=(0.3, 3.3), value=0, inplace=False):
        self.worker = T.Compose([
            T.ToTensor(),   # PIL → Tensor
            T.RandomErasing(
                p=p, scale=scale, ratio=ratio, value=value, inplace=inplace
            ),
            T.ToPILImage()  # Tensor → PIL (to keep pipeline consistent)
        ])
...
class GroupRandomRotation(object):
    def __init__(self,degrees):
        self.worker = torchvision.transforms.RandomRotation(degrees=degrees)
    def __call__(self, img_group):
        #print("rando rotatio")
        return [self.worker(img) for img in img_group]
