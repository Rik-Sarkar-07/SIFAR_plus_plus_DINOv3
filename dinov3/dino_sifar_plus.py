...
class DinoVisionTransformer(nn.Module):
    def __init__(
        ...
        **ignored_kwargs,
        ...
        #-------------- Temporal Prompthing -----------------------------#
        self.num_frames = 16
        self.temporal_prompt = nn.Parameter(torch.randn(1, self.num_frames, embed_dim))
        #---------------------------------------------------------------#
    ...
    def prepare_tokens_with_masks(self, x: Tensor, masks=None) -> Tuple[Tensor, Tuple[int]]:
        ...
        """
        temporal_prompt = self.temporal_prompt.expand(B, -1, -1)

        x = torch.cat(
            [
                cls_token.expand(B, -1, -1),
                storage_tokens.expand(B, -1, -1),
                temporal_prompt,   # ← added line
                x,
            ],
            dim=1,
        )
    ...
    def forward_features(self, x: Tensor | List[Tensor], masks: Optional[Tensor] = None) -> List[Dict[str, Tensor]]:
        ...
        if input_size != to_2tuple(img_size):
            x = nn.functional.interpolate(x, size=img_size,mode='bilinear')        
        x = rearrange(x, 'b (th tw c) h w -> b c (th h) (tw w)', th=4, c=3)
