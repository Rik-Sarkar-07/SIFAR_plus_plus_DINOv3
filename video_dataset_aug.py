...
from video_transforms import (GroupRandomHorizontalFlip, GroupOverSample,
                               ...
                               GroupNormalize, Stack, ToTorchFormatTensor, GroupRandomScale, GroupRandomRotation, GroupGaussianBlur, GroupRandAugment, GroupRandomErasing)
...
def get_augmentor(is_train: bool, image_size: int, mean: List[float] = None,
                  ...
                  modality: str = 'rgb', num_clips: int = 1, num_crops: int = 1, dataset: str = '', scales: List[float] = [1, .975, .95, .925], #[1, .875, .75, .66], 
                  randomrotation: int = 0, blur: bool = False, randarg: bool = False, erasing: bool = False):
    ...
    if modality == 'sound':
        ...
        if is_train:
            if blur:
                augments += [GroupGaussianBlur(kernel_size=9, sigma=(0.1, 2.0))]
            # randarg = True
            if randarg:
                augments += [GroupRandAugment(6,6)]
                print("Applying the GroupRandAugment")
            if erasing:
                augments += [GroupRandomErasing()]
                print("Applying the GroupRandAugment with p = 0.25")
            if (randomrotation > 0):
            #print("GroupRandomRotation = ", randomrotation)
                augments += [GroupRandomRotation(randomrotation)]
            if version == 'v1':
                augments += [
                    GroupMultiScaleCrop(image_size, scales) # GroupMultiScaleCrop(image_size, [1, .875, .75, .66]).3)
                ]
            elif version == 'v2':
                augments += [
            ...
            if not (dataset.startswith('st2stv') or 'jester' in dataset or 'mini_ststv' in dataset):
...
def build_dataflow(dataset, is_train, batch_size, workers=36, is_distributed=False):
    ...
    data_loader = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=shuffle,
                                              num_workers=workers, pin_memory=False, sampler=sampler, drop_last=True)
