...
# Modify by Sudipta Sarkar on 15 February 2026 for MSN finetuning on video classification tasks.
...
def train_one_epoch(model: torch.nn.Module, run, wandb, criterion: torch.nn.Module,
                    ...
                    finetune=False, msn_pretraining=False
                    ):
    ...
    for samples, targets in metric_logger.log_every(data_loader, print_freq, header):
        ...
        if wandb and utils.is_main_process():
            run.log({"lr": optimizer.param_groups[0]["lr"], "train_loss": loss_value, "train_grad_norm": grad_norm, "train_weight_norm": weight_norm })
...
@torch.no_grad()
def evaluate(data_loader, model, run, wandb, device, world_size, distributed=True, amp=False, num_crops=1, num_clips=1):
    ...
    if wandb and utils.is_main_process():
        run.log({"test_loss": real_loss, "test_acc1": real_acc1.item(), "test_acc5": real_acc5.item() })
