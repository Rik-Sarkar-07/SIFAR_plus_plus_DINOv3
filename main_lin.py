...
def main(args):
   ...
   #  ----------------------------------------------  Model creation ---------------------------------------------------------------- #
    ...
    # -------------------------------------------------------------------------------------------- #
    # Remove original head BEFORE loading pretrained weights
    ...
    if args.msn_pretraining:
        print("Loading MSN DINOv3 pretrained weights from:", args.msn_model_path)
        ckpt = torch.load(args.msn_model_path, map_location="cpu")
    ...
    elif args.dino_model_path:
        print("Loading LDV pretrained DINOv3 weights from:", args.dino_model_path)
   ...
   # ---------------------------- Linear Evaluation Setup ---------------------------- #
    ...
    # 2️ Replace classifier
    model.head = nn.Linear(model.embed_dim, args.class_numbers)
    torch.nn.init.trunc_normal_(model.head.weight, std=0.02)
    torch.nn.init.zeros_(model.head.bias)

    # 3️ Unfreeze ONLY head
    for param in model.head.parameters():
        param.requires_grad = True

    # 4️ Extra safety (important)
    assert all(not p.requires_grad for n, p in model.named_parameters() if "head" not in n)
    assert all(p.requires_grad for n, p in model.named_parameters() if "head" in n)

    print("\n===== Linear Evaluation Mode =====")
    print("Trainable parameters:",
        [n for n, p in model.named_parameters() if p.requires_grad])

    print("Number trainable:",
