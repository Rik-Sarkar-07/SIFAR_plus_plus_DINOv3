...
# Modify by Sudipta Sarkar on date 15 February 2026 for MSN finetuning on video classification tasks.
...
from video_dataset_orig import VideoDataSet, VideoDataSetLMDB, VideoDataSetOnline
...
from dinov3.vision_transformer import vit_small, vit_base
...
def get_args_parser():
    ...
    #parser.add_argument('--randomarg', type=bool, default=False, help='Random Augment true or false')
    ...
    # model specific
    parser.add_argument('--model', default='dino_small', type=str, help='Name of model to train')
    parser.add_argument('--class_numbers', type=int, default=400, help='Num classes')
    parser.add_argument('--dino_model_path', type=str, default="", help='LVD pretrained dino model path for loading weights without MSN')
    parser.add_argument('--msn_model_path', type=str, default="", help='MSN pretrained model path for loading weights with MSN')
    parser.add_argument('--msn_pretraining', type=bool, default=False, help='True if loading MSN pretrained weights, false if loading LDV pretrained DINOv3 weights without MSN')
    parser.add_argument('--ldecay', type=float, default=0.75, help='0.75') 
...
#------------------------------------------------- Layer-wise Learning Rate Decay (LLRD) [Default value 0.75, set in parameters] by Sudipta---------------------------#

import re
from collections import defaultdict

def get_parameter_groups_with_llrd(model, base_lr, weight_decay=0.05, layer_decay=0.75, verbose=True):
    """
    Build parameter groups with Layer-wise LR Decay (LLRD) for ViT-like models.

    Args:
        model: nn.Module (your vit_models instance or wrapped DDP model.module)
        base_lr: float, LR for the top (head) layer
        weight_decay: float
        layer_decay: float in (0,1) to decay LR for lower layers
        verbose: print debug info on rank 0 if in distributed mode
    Returns:
        list of param-group dicts (ready for torch.optim.{AdamW,SGD,...})
    """
    # If model is DDP wrapper, obtain the underlying module for layer counting
    plain_model = getattr(model, "module", model)

    # number of transformer blocks
    num_blocks = len(plain_model.blocks) if hasattr(plain_model, "blocks") else 12
    max_layer_id = num_blocks + 1  # 0..num_blocks for blocks, +1 for head

    # helper: find blocks.<idx> pattern
    block_re = re.compile(r"blocks\.([0-9]+)")

    # accumulate params per (layer_id, decay_flag)
    groups = defaultdict(lambda: {"params": [], "weight_decay": weight_decay})

    for name, param in plain_model.named_parameters():
        if not param.requires_grad:
            continue

        # default layer id = num_blocks (put into high-level group)
        layer_id = num_blocks
        m = block_re.search(name)
        if m:
            layer_id = int(m.group(1)) + 1  # blocks.0 -> layer 1
        elif name.startswith(("cls_token", "pos_embed", "patch_embed")):
            layer_id = 0
        elif name.startswith("head") or name.endswith("head.weight") or name.endswith("head.bias"):
            layer_id = max_layer_id

        # decide weight decay for this param (bias & norm params -> no wd)
        if param.ndim == 1 or name.endswith(".bias") or any(x in name.lower() for x in ("norm", "ln", "layernorm", "bias")):
            wd = 0.0
        else:
            wd = weight_decay
        ...
        groups[key]["weight_decay"] = wd  # keep the proper wd

    # convert to param_groups list, computing LR per layer
    param_groups = []
    for (layer_id, wd), val in sorted(groups.items(), key=lambda x: (x[0][0], -x[0][1])):
        # compute scale: deeper layers get larger LR. We map layer_id so that head (max_layer_id) has scale 1.0
        # Using exponent = (max_layer_id - layer_id)
        scale = layer_decay ** (max_layer_id - layer_id)
        lr = base_lr * scale
        pg = {"params": val["params"], "lr": lr, "weight_decay": wd}
        param_groups.append(pg)

    # optional small debug: print lr per layer & param counts (only on rank 0)
    try:
        is_main = True
        import torch.distributed as _dist
        if _dist.is_available() and _dist.is_initialized():
            is_main = (_dist.get_rank() == 0)
    except Exception:
        is_main = True

    if verbose and is_main:
        print("----- LLRD parameter groups summary -----")
        # summarize by layer id (aggregate groups with same layer_id)
        summary = defaultdict(lambda: {"params": 0, "wds": set(), "lrs": set()})
        for (layer_id, wd), val in sorted(groups.items(), key=lambda x: x[0]):
            scale = layer_decay ** (max_layer_id - layer_id)
            lr = base_lr * scale
            summary[layer_id]["params"] += sum(p.numel() for p in val["params"])
            summary[layer_id]["wds"].add(wd)
            summary[layer_id]["lrs"].add(lr)
        for lid in sorted(summary.keys()):
            linfo = summary[lid]
            print(f"Layer {lid:2d} — params: {linfo['params']:,}  lr(s): {sorted(linfo['lrs'])}  wd(s): {sorted(linfo['wds'])}")
        print("-----------------------------------------")
...
def main(args):
    ...
    if args.wandb and utils.is_main_process():
        ...
        run = wandb.init(
            ...
            project="my-project-owais-finetune",
    ...
    # Remove classification head BEFORE loading
    model.head = nn.Identity()
    # Now add your classifier head
    model.head = nn.Linear(model.embed_dim, args.class_numbers)
    ...
    if args.msn_pretraining:
        model_load_path = args.msn_model_path
        print("Loading MSN Dinov3 pretrainied weight from :",model_load_path)
        print("Num of classes:",args.class_numbers)
        ...
        if args.dino_model_path:
            print("Loading LDV pretrained DINOv3 weights without MSN from :",args.dino_model_path)
            ckpt = torch.load(args.dino_model_path,  map_location="cpu", weights_only=True)
    ...
    #---------------------------------------- Calling Layer-wise Learning Rate Decay (LLRD)-----------------------#
    ...
    train_augmentor = get_augmentor(True, args.input_size, mean, std, threed_data=args.threed_data,
                                    version=args.augmentor_ver, scale_range=args.scale_range, dataset=args.dataset, randomrotation=args.randomrotation, blur=args.blur)
    ...
    if args.eval:
        test_stats = evaluate(data_loader_val, model, run, args.wandb, device, num_tasks, distributed=True, amp=args.amp, num_crops=args.num_crops, num_clips=args.num_clips)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            model, run, args.wandb, criterion, data_loader_train,
        ...
        test_stats = evaluate(data_loader_val, model, run, args.wandb, device, num_tasks, distributed=True, amp=args.amp)
        print(f"Accuracy of the network on the {len(dataset_val)} test images: {test_stats['acc1']:.1f}%")
